...
def get_bal_ratio_step(bal_ratio: float) -> float:
    ...
    if abs(bal_ratio) < 0.05:
        ...
        bal_ratio_step = bal_ratio * 0.4
    elif abs(bal_ratio) < 0.4:  # bal_ratio = 0.3, bal_ratio_step = 0.15
        bal_ratio_step = bal_ratio * 0.7
    else:
        bal_ratio_step = 1
...
def get_risk_adjusted_bid_ask_micro_version(bal_ratio: float, fair_price: float, bid_px: float, ask_px: float) -> float:
    ...
    # fee adjustment
    bid_micro, ask_micro = add_fee(bid_micro, ask_micro, fee)
...
def add_fee(bid_px, ask_px, fee):
    return bid_px * (1 - fee), ask_px * (1 + fee)
...
def get_mm_price(sym: str, mkt_snap, risk) -> typing.Tuple[float, float]:
    ...
    # bal_raio = 0.45 means we try to hold 45% of eth
    bal_ratio = risk.get_bal_ratio(sym) - 0.5  # eth bal ratio
...
def make_markets(session, mkt_snap, risk, orders) -> dict:
    ...
    if orders[sym]:
        ...
        bid_move = False if float(
            abs(bid_price_us - bid_us['price']) / bid_us['price']) < 0.0008 else True
        ask_move = False if float(
            abs(ask_price_us - ask_us['price']) / ask_us['price']) < 0.0008 else True
        ...
        x = current_timestamp - order_info['tick']
        print(f'safe: {safe}')
        print(f'stamp pass: {x}')
        ...
        if (bid_status == 'open' and bid_move) or (bid_status == 'closed' and safe):
            ...
            if bid_status == 'open':
                try:
                    ...
                    print('error cancelling bid, already filled')
        ...
        if (ask_status == 'open' and ask_move) or (ask_status == 'closed' and safe):
            ...
            if ask_status == 'open':
                try:
                    ...
                    print('error cancelling ask, already filled')
